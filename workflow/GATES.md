...
| 2B | SME PRE-BUILD | Expert validation | SCOPE_LOCK.md updated | Yes (OVERRIDE) |
| 3 | ARCHITECTURE | Lock architecture | ARCHITECTURE.md | Yes (simple features) |
| 4 | EXECUTION | Build features | prd.json, code | No |
| 5 | VALIDATION | Score check | SCORE.json | No |
| 5B | SME PRE-SHIP | Expert ship review | SHIP_VALIDATION.md | Yes (ACCEPT-RISK) |
...
```
Gate 0 ──→ Gate 1 ──→ Gate 2 ──→ Gate 2B ──→ Gate 3 ──→ Gate 4 ──→ Gate 5 ──→ Gate 5B ──→ DEPLOY
  │           │                     │           │                       │
  │           ▼                     ▼           ▼                       ▼
  │      [SKIP-MARKET]         [OVERRIDE]   [SKIP-ARCH]           [ACCEPT-RISK]
  │           │                     │           │                       │
  ▼           ▼                     ▼           ▼                       ▼
[Brownfield joins here]─────────────────────────→                   [ITERATE]
                                                                        │
                                                                        ▼
                                                                    Gate 4 (loop)
```
...
## SME Validation Points

### Gate 2B: SME PRE-BUILD (after scope lock)

**Purpose:** Challenge assumptions before building.

**Checks:**
- Market fit assumptions
- Technical feasibility
- User behavior assumptions
- Riskiest assumptions
- Hidden dependencies

**Skip:** Type `OVERRIDE` per question

---

### Gate 5B: SME PRE-SHIP (after validation score)

**Purpose:** Final expert review before deployment.

**Checks:**
- Outcome delivered vs SCOPE.md
- Would persona actually use this?
- Missing blockers for adoption
- Bugs, UX issues, edge cases
- Production vs prototype grade
- Production risks, blast radius
- Rollback capability

**Skip:** Type `ACCEPT-RISK` per concern

**Output:** SHIP_VALIDATION.md with quality grade and risk level
