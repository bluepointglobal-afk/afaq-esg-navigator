...
<body>
    <header>
        <h1>${isRtl ? 'تقرير الإفصاح عن الاستدامة' : 'ESG Disclosure Report'}</h1>
        <div class="meta">
            <strong>${disclosure.generatedForCompany}</strong><br>
            ${isRtl ? 'السنة المالية' : 'Fiscal Year'}: ${disclosure.templateVersion || '2024'} | 
            ${isRtl ? 'تاريخ الإصدار' : 'Issue Date'}: ${new Date(disclosure.generatedAt).toLocaleDateString(language === 'ar' ? 'ar-AE' : 'en-US')}
        </div>
    </header>
    ...
    <div class="footer">
        ...
        Report ID: ${disclosure.id.substring(0, 8)} | 
        Authenticity Check: ${btoa(disclosure.id).substring(0, 12)}...
    </div>
</body>
</html>
  `;
...
<body>
    ...
    <footer style="margin-top: 60px; font-size: 12px; color: #94a3b8; text-align: center;">
        This appendix accompanies ESG Disclosure Report ${disclosure.id.substring(0, 8)}.
    </footer>
</body>
</html>
  `;
...
export function renderSustainabilityReportToHtml(
    ...
    .map(section => `
      <section class="report-section" style="--section-color: ${sectionColors[section.type] || '#475569'}">
        <div class="section-header">
          <span class="section-icon">${sectionIcons[section.type] || '📄'}</span>
          <div>
            <h2>${section.title}</h2>
  ...
  const dataAnnexHtml = report.dataAnnex.length > 0 ? `
    <section class="data-annex">
      <h2>📊 Data Annex - Quantitative Disclosures</h2>
...
<body>
    <header>
        ...
        <div class="report-meta">
            Sustainability Report ${report.reportingYear} • ${report.jurisdiction}
        </div>
    ...
    <footer>
        ...
        <p>Template Version ${report.templateVersion} • Report ID: ${report.id.substring(0, 8)}</p>
